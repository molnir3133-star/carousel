  ...
  return (
    <div style={{ 
      ...
      <div style={{ position: 'absolute', bottom: '30px', textAlign: 'center', width: '100%' }}>
        <p style={{ fontSize: '10px', color: '#ccc', letterSpacing: '2px', marginBottom: '10px' }}>WISHLIST</p>
        <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', minHeight: '50px', overflowX: 'auto', padding: '0 20px' }}>
          <AnimatePresence>
            {likedProducts.map((p) => (
              <motion.img 
                key={p.url}
                ...
                style={{ width: '45px', height: '45px', borderRadius: '50%', objectFit: 'cover', border: '1px solid #eee', cursor: 'pointer' }} 
              />
            ))}
          </AnimatePresence>
        </div>
